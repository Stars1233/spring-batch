...
By default, the writer calls `CrudRepository#saveAll(Iterable)` with the whole chunk.
Another method can be selected with `setMethodName`. If the named method accepts a
single item, it is invoked once per item in the chunk. If it accepts an `Iterable`
instead (for example `JpaRepository#saveAllAndFlush(Iterable)`), it is invoked once with
the whole chunk.

If the underlying repository needs to be flushed after writing and does not expose a
single method that both saves and flushes (unlike `saveAllAndFlush`), the `flush` method
can be overridden in a subclass to perform the flush after the chunk has been written.
The `repository` field is `protected` so that subclasses can access it directly:
